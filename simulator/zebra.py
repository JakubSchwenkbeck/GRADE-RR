...
try:
	...
	# Modular pipeline initialization
	...
	# Initialize modular pipeline module
	replicator_pipeline = ReplicatorPipeline(config=config, out_dir=out_dir)
	replicator_pipeline.setup_data_writer()
	...
	while kit.is_running():
		...
		if interactive_preview_mode:
			if simulation_step == 0:
				...
				zebra_keys = [k for k in frame_info_preview.keys() if k.startswith("/zebra") and "position" in frame_info_preview[k]]
				
				if zebra_keys:
					# Dynamically compute and execute tracked cameras through the pipeline
					target_zebra_path = max(zebra_keys, key=lambda k: frame_info_preview.get(k, {}).get("position", [0, 0, -1e9])[2])
                    
					zebra_pos_m = np.array(frame_info_preview[target_zebra_path]["position"])
					replicator_pipeline.update_tracking_cameras(zebra_pos_m)
		...
		zebra_keys = [k for k in frame_info.keys() if k.startswith("/zebra") and "position" in frame_info.get(k, {})]
		num_cameras = len(viewport_window_list)
		
		if zebra_keys and num_cameras > 0:
			from isaacsim.core.utils.xforms import get_world_pose
			
			target_zebra_path = max(zebra_keys, key=lambda k:frame_info.get(k, {}).get("position", [0, 0, -1e9])[2])
			zebra_live_pos, _ = get_world_pose(target_zebra_path)
			
			if np.max(np.abs(zebra_live_pos)) > np.max(np.abs(frame_info[target_zebra_path]["position"])) * 5.0:
				zebra_pos_m = np.array(zebra_live_pos) * meters_per_unit
			else:
				zebra_pos_m = np.array(zebra_live_pos)
			
			frame_info[target_zebra_path]["position"] = zebra_pos_m.tolist()
			
			# Execute side profile staggered views via our imported tracking routine
			replicator_pipeline.update_tracking_cameras(zebra_pos_m, viewport_window_list)
			
			if simulation_step % 10 == 0:
				print(f"[INFO] Frame {simulation_step}: Successfully locked onto active asset {target_zebra_path}.")

			# Hand current iteration vectors off to the pipeline metadata layer
			frame_info["step"] = simulation_step
			frame_info["substep"] = 0
			replicator_pipeline.assign_metadata(simulation_step, zebra_pos_m)
		...
		for c_substep in range(substep):
			...
			if config["record"].get():
				...
				# Track target asset metrics if tracking arrays are populated
				target_zebra_pos = None
				if zebra_keys:
					target_zebra_path = max(zebra_keys, key=lambda k: frame_info.get(k, {}).get("position", [0, 0, -1e9])[2])
					target_zebra_pos = np.array(frame_info[target_zebra_path]["position"])
				
				# Log spatial metadata tracking frame entries 
				replicator_pipeline.assign_metadata(simulation_step, target_zebra_pos)
                
				# Evaluate pipeline runtime frames natively using pipeline replicator instance
				replicator_pipeline.rep.orchestrator.step(rt_subframes=preview_inner_renders, delta_time=0.0)
